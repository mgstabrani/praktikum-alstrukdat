...
#include "stdlib.h"
#include "stdio.h"
#include "../src/arraydin.h"

#define IdxMax 200

void IsiTabel(TabInt *T);
/* I.S. T sembarang */
/* F.S. Terbentuk tabel T dengan 5 elemen pertama berupa 1, 2, 3, 4, 5 */

void IsiTabelElemenSama(TabInt *T);
/* I.S. T sembarang */
/* F.S. Terbentuk tabel T dengan 5 elemen bernilai 2 */
...
// Output: Semua elemen T 
  TabInt T1;
  MakeEmpty(&T1, IdxMax);
  ck_assert_int_eq(0, Neff(T1));
  ck_assert_int_eq(IdxMax, MaxEl(T1));
} END_TEST 
...
// Input: T berisi 5 elemen
// Output: 5 (jumlah elemen)
  TabInt T1;
  IsiTabel(&T1);
  ck_assert_int_eq(5, NbElmt(T1));
} END_TEST

START_TEST(MaxElement_TC1) {
// Input: T berisi 5 elemen
// Output: IdxMax
  TabInt T1;
  MakeEmpty(&T1, IdxMax);
  ck_assert_int_eq(IdxMax, MaxElement(T1));
} END_TEST
...
// Output: IdxMin
  TabInt T1;
  IsiTabel(&T1);
  ck_assert_int_eq(4, GetLastIdx(T1));
} END_TEST
...
START_TEST(IsIdxValid_TC3) {
// Input: T terisi, Maxel = 200
// Output: true
  TabInt T1;
  IsiTabel(&T1);
  ck_assert_int_eq(true, IsIdxValid(T1, 50));
} END_TEST

START_TEST(IsIdxEff_TC1) {
// Input: T berisi 5 elemen, Index 4
// Output: true
  TabInt T1;
  IsiTabel(&T1);
  ck_assert_int_eq(true, IsIdxEff(T1, 4));
} END_TEST

START_TEST(IsIdxEff_TC2) {
// Input: T berisi 5 elemen, Index -1
// Output: false
  TabInt T1;
  IsiTabel(&T1);
...
// Output: IdxUndef
  TabInt T1;
  IsiTabel(&T1);
  ck_assert_int_eq(IdxUndef, Search1(T1, 7));
} END_TEST
...
// Output: true
  TabInt T1;
  IsiTabel(&T1);
  ck_assert_int_eq(false, SearchB(T1, 7));
} END_TEST
...
// Output: 1 nilai maksimum, 1 nilai minimum
  ...
  MaxMin(T1, &Max, &Min);
  ck_assert_int_eq(1, Min);
  ck_assert_int_eq(200, Max);
} END_TEST

/* ********** OPERASI LAIN ********** */
START_TEST(CopyTab_TC1) {
// Input: T1 Kosong
// Output: 0
  TabInt T1, T2;
  IsiTabel(&T1);
  CopyTab(T1, &T2);
  ck_assert_int_eq(Elmt(T2, 2), Elmt(T1, 2));
} END_TEST

START_TEST(CopyTab_TC2) {
// Input: T1 Kosong
// Output: 0
  TabInt T1, T2;
  IsiTabel(&T1);
  CopyTab(T1, &T2);
  ck_assert_int_eq(Neff(T2), Neff(T1));
} END_TEST

START_TEST(CopyTab_TC3) {
// Input: T1 Kosong
// Output: 0
  TabInt T1, T2;
  IsiTabel(&T1);
  CopyTab(T1, &T2);
  ck_assert_int_eq(MaxEl(T2), MaxEl(T1));
} END_TEST
...
// Input: T1 berisi 5 elemen
// Output: 6
  TabInt T1;
  IsiTabel(&T1);
  ck_assert_int_eq(15, SumTab(T1));
} END_TEST
...
// Input: T1 berisi 5 elemen, ElType merupakan elemen T1
// Output: Jumlah elemen tersebut
  TabInt T1;
  IsiTabelElemenSama(&T1);
  ck_assert_int_eq(5, CountX(T1, 2));
} END_TEST

START_TEST(CountX_TC3) {
// Input: T1 berisi 5 elemen, ElType bukan merupakan elemen T1
// Output: Jumlah elemen tersebut
  TabInt T1;
  IsiTabelElemenSama(&T1);
...
// Input: T1 berisi 5 elemen yang semuanya genap
// Output: true
  TabInt T1;
  IsiTabelElemenSama(&T1);
...
// Input: T1 berisi 5 elemen yang sebagian tidak genap
// Output: false
  TabInt T1;
  IsiTabel(&T1);
...
// Output: T1 terurut membesar
  ...
  for (i = GetFirstIdx(T1); i <= GetLastIdx(T1); i++) {
    ck_assert_int_eq(i + 2, Elmt(T1, i));
  }
} END_TEST
...
// Output: T1 terurut mengecil
  ...
  Sort(&T1, false);
  for (i = GetFirstIdx(T1); i <= GetLastIdx(T1); i++) {
    ck_assert_int_eq(GetLastIdx(T1) - i + 2, Elmt(T1, i));
  }
} END_TEST

/* ********** MENAMBAH DAN MENGHAPUS ELEMEN DI AKHIR ********** */
START_TEST(AddAsLastEl_TC1) {
// Input: T1 berisi 5 elemen, ElType sembarang
...
START_TEST(GrowTab_TC1) {
// Input: T1 berisi 3 elemen, ElType sembarang
// Output: T1 kehilangan 1 elemen ElType, X berisi ElType elemen terakhir
  TabInt T1;
  MakeEmpty(&T1, IdxMax);
  GrowTab(&T1, 5);
  ck_assert_int_eq(IdxMax + 5, MaxEl(T1));
} END_TEST

START_TEST(ShrinkTab_TC1) {
// Input: T1 berisi 3 elemen, ElType sembarang
// Output: T1 kehilangan 1 elemen ElType, X berisi ElType elemen terakhir
  TabInt T1;
  IsiTabel(&T1);
  ShrinkTab(&T1, 5);
  ck_assert_int_eq(195, MaxEl(T1));
} END_TEST

START_TEST(CompactTab_TC1) {
// Input: T1 berisi 3 elemen, ElType sembarang
// Output: T1 kehilangan 1 elemen ElType, X berisi ElType elemen terakhir
  TabInt T1;
  IsiTabel(&T1);
  CompactTab(&T1);
  ck_assert_int_eq(5, MaxEl(T1));
} END_TEST
...
Suite *arraypos_suite(void) { 
  ...
  tcase_add_test(tc_selektor, MaxElement_TC1);
  ...
  tcase_add_test(tc_selektor, IsIdxValid_TC3);
  ...
  tcase_add_test(tc_operasiLain, CopyTab_TC1);
  tcase_add_test(tc_operasiLain, CopyTab_TC2);
  tcase_add_test(tc_operasiLain, CopyTab_TC3);
  ...
  tc_operasiUkuran = tcase_create("Operasi Ukuran");
  tcase_add_test(tc_operasiUkuran, GrowTab_TC1);
  tcase_add_test(tc_operasiUkuran, ShrinkTab_TC1);
  tcase_add_test(tc_operasiUkuran, CompactTab_TC1);
  suite_add_tcase(s, tc_operasiUkuran);
